...
export class RustUp {
  ...
  public static async getOrInstall(): Promise<RustUp> {
    try {
      ...
      core.debug(
        `Unable to find "rustup" executable, installing it now. Reason: ${String(
          error
        )}`
      );
  ...
  public async supportProfiles(): Promise<boolean> {
    ...
    if (supports) {
      ...
      core.info(
        `Installed rustup ${version} does not support profiles, \
expected at least ${PROFILES_MIN_VERSION}`
  ...
  public async supportComponents(): Promise<boolean> {
    ...
    if (supports) {
      ...
      core.info(
        `Installed rustup ${version} does not support components, \
expected at least ${PROFILES_MIN_VERSION}`
      );
    }
    return supports;
